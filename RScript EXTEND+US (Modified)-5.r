...
write.table(l_cor_nprobes_df, "table (Split 5).txt", row.names=T, col.names=T, quote=F)



# Plot (+Save) correlation against number of weighted probes
pdf("Plot (Split 5).pdf")
...
write.table(l_rmse_df, "RMSE (Split 5).txt", row.names=T, col.names=T, quote=F)
...
write.table(probes_model, "probes_model (Split 5).txt", row.names=T, col.names=T, quote=F)
...
write.table(df_final, "probes_coef (Split 5).txt", row.names=T, col.names=T, quote=F)
